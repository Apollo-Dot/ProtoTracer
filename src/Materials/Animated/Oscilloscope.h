...
private:
    ...
    MaxFilter<30> maxF = MaxFilter<30>();
    MinFilter<30> minF = MinFilter<30>();
...
public:
    ...
    void Update(float* data){
        ...
        float minValue = minF.Filter(data[bins / 2 + 1]);
        float maxValue = maxF.Filter(data[bins / 2 + 1]);
    ...
    RGBColor GetRGB(Vector3D position, Vector3D normal, Vector3D uvw) override {
        ...
        uint8_t x = uint8_t(Mathematics::Map(rPos.X, -size.X, size.X, float(bins - 1), float(bins - 1) / 2 + 20));
